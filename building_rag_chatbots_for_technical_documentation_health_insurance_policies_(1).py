# -*- coding: utf-8 -*-
"""Building_RAG_Chatbots_for_Technical_Documentation_Health_Insurance_Policies (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1l5edPULeLYguREMpVbTIfnYIFGt6qu9d
...
loader = UnstructuredHTMLLoader(file_path="/content/How to use the various modes of the washing machine _ Samsung LEVANT.html")
...
os.environ["OPENAI_API_KEY"] = "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"
...
## Downloading the Star Health Insurance Manual

First, we need to download the HTML content from the Star Health Insurance website. We'll use the `requests` library for this and save the content to a new HTML file.
"""

import requests

health_insurance_url = "https://www.starhealth.in/health-insurance/types-of-health-insurance/"
output_filename = "star_health_insurance.html"

try:
    response = requests.get(health_insurance_url)
    response.raise_for_status() # Raise an exception for HTTP errors
    with open(output_filename, "w", encoding="utf-8") as f:
        f.write(response.text)
    print(f"Successfully downloaded '{health_insurance_url}' to '{output_filename}'")
except requests.exceptions.RequestException as e:
    print(f"Error downloading the page: {e}")

"""Now that we have the Star Health Insurance manual downloaded, we need to re-initialize the `UnstructuredHTMLLoader` to load this new file. Then, we will re-process the documents, split them into chunks, and create a new Chroma vectorstore and retriever based on this health insurance data. The `rag_chain` will automatically use the updated retriever."""

# Load the new HTML file as a LangChain document
loader = UnstructuredHTMLLoader(file_path=output_filename)
health_insurance_docs = loader.load()

# Initialize RecursiveCharacterTextSplitter
text_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200)

# Split the health insurance documents
splits = text_splitter.split_documents(health_insurance_docs)

# Initialize Chroma vectorstore with new documents and embeddings
vectorstore = Chroma.from_documents(documents=splits, embedding=embeddings)

# Setup new vectorstore as retriever
retriever = vectorstore.as_retriever()

print("LangChain components re-initialized with Star Health Insurance data.")

# You can now test with the sample queries provided earlier for health insurance data.
# For example:
# query_health = "What are different Maternity Health Insurance Plan in StarHealth?"
# answer_health = rag_chain.invoke(query_health).content
# print(answer_health)